    ...
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
      <div v-for="school in filteredSchools" :key="school.id" class="card hover:shadow-lg transition-shadow">
        <div class="h-40 sm:h-48 bg-gray-200 rounded-t-lg overflow-hidden relative">
          ...
          />
          <div v-else class="flex items-center justify-center h-full bg-skyway text-white">
            <img src="/images/placeholders-png/school-logo.png" alt="Логотип" class="w-32 h-32 sm:w-48 sm:h-48" />
          </div>
        </div>
        ...
        <div class="p-3 sm:p-4">
          ...
          <div class="flex items-center text-gray-500 mb-4 text-sm">
            ...
            <span>{{ school.programs.length }} направлений обучения</span>
          </div>
          ...
          <div class="flex justify-between items-center">
            ...
            <button 
              ...
              :class="{ 'text-red-500': savedSchools.includes(school.id) }"
              :title="savedSchools.includes(school.id) ? 'Удалить из сохраненных' : 'Сохранить'"
              aria-label="Сохранить учебное заведение"
            >
              <svg class="w-6 h-6" :fill="savedSchools.includes(school.id) ? 'currentColor' : 'none'" viewBox="0 0 24 24" stroke="currentColor">
...
const extractPrograms = (schools) => {
  const programsSet = new Set()
  
  schools.forEach(school => {
    if (school.programs && Array.isArray(school.programs)) {
...
const filteredSchools = computed(() => {
  ...
  if (filters.program) {
    result = result.filter(school => 
      school.programs && Array.isArray(school.programs) && 
      school.programs.some(program => program && program.name === filters.program)
    )
  }
...
// Сохранение/удаление школы из избранного
const toggleSaveSchool = async (school) => {
  ...
  const isSaved = savedSchools.value.includes(school.id)
  const method = isSaved ? 'DELETE' : 'POST'
  
  try {
    const response = await fetch(`/api/schools/${school.id}/save`, {
      method,
      headers: {
    ...
    if (response.ok) {
      if (isSaved) {
        ...
        savedSchools.value = savedSchools.value.filter(id => id !== school.id)
      } else {
        // Добавляем в сохраненные
        savedSchools.value.push(school.id)
      }
    } else {
...
const loadSavedSchools = async () => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data.body && data.body.savedSchools) {
        savedSchools.value = data.body.savedSchools.map(saved => saved.schoolId)
      }
    }
  } catch (error) {
...
onMounted(async () => {
  try {
    ...
    if (data && data.body) {
      ...
      programs.value = extractPrograms(schools.value)
